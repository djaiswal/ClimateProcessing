...
source("/home/a-m/djaiswal/Scripts/ClimateProcessing/ISIMIP/getFirstIndex_for_isimip.R")
...
filename<-paste(maindirectory,modelname,"/tas_bced_1960_1999_miroc-esm-chem_",scenarioname,"_",yearinterval[3],".nc", sep="")
tmp0<-nc_open(filename)
tavg<-ncvar_get(tmp0)
nc_close(tmp0)

#Reading maximum temperature
filename<-paste(maindirectory,modelname,"/tasmax_bced_1960_1999_miroc-esm-chem_",scenarioname,"_",yearinterval[3],".nc", sep="")
tmp0<-nc_open(filename)
tmax<-ncvar_get(tmp0)
nc_close(tmp0)

#Reading minimum temperature
filename<-paste(maindirectory,modelname,"/tasmin_bced_1960_1999_miroc-esm-chem_",scenarioname,"_",yearinterval[3],".nc", sep="")
tmp0<-nc_open(filename)
tmin<-ncvar_get(tmp0)
nc_close(tmp0)

#Reading precipitation
filename<-paste(maindirectory,modelname,"/pr_bced_1960_1999_miroc-esm-chem_",scenarioname,"_",yearinterval[3],".nc", sep="")
tmp0<-nc_open(filename)
pr<-ncvar_get(tmp0)
nc_close(tmp0)

#Reading  wind
filename<-paste(maindirectory,modelname,"/wind_bced_1960_1999_miroc-esm-chem_",scenarioname,"_",yearinterval[3],".nc", sep="")
tmp0<-nc_open(filename)
wind<-ncvar_get(tmp0)
nc_close(tmp0)
...
#Shortwave radiation
filename<-paste(maindirectory,modelname,"/rsds_bced_1960_1999_miroc-esm-chem_",scenarioname,"_",yearinterval[3],".nc", sep="")
tmp0<-nc_open(filename)
sRad<-ncvar_get(tmp0)
nc_close(tmp0)
...
#Relative humidity
filename<-paste(maindirectory,modelname,"/hurs_miroc-esm-chem_",scenarioname,"_",yearinterval[3],".nc", sep="")
tmp0<-nc_open(filename)
rhs<-ncvar_get(tmp0)
...
####################################################################################################
 ...
 # getting rid of missing/strange values (very small negative) of other troublesome climate data to avoid error in simulation
            ...
            #Now we are ready to make dataframe to call weach
              ...
              filename=paste("/home/groups/ebimodeling/deepak_tmp/ISIMIP/",modelname,"/Processed/",year,formatC(ii,width=3,flag=0),formatC(jj,width=3,flag=0),".RData",sep="")
